...
صف `/admin/ads/pending` در `PendingAdsView.vue` یک DataTable کارت‌محور با ستون‌های آگهی‌دهنده و وضعیت هویت، عنوان/بانک/طرح، نوع معامله، مبالغ، موقعیت، زمان ثبت و عملیات فوری دارد. نوار فیلتر جدید به‌صورت RTL و کارت سفید PrimeVue شامل `Dropdown` بانک عامل، `Dropdown` نوع معامله، `Dropdown` بازه مبلغ، `InputText` جست‌وجوی هوشمند و دکمه‌های پاک‌سازی و اعمال فیلتر است. فیلتر جست‌وجو عنوان، کد آگهی، نام کاربر و موبایل را پوشش می‌دهد و state واکنشی `bank`, `deal_type`, `amount_range`, `search` را به query سرور می‌فرستد. درخواست `GET /api/admin/ads/pending` پارامترهای `bank`/`bank_id`, `deal_type`/`type`, `amount_range` و `search` را validate می‌کند؛ بازه‌ها روی `loan_amount` تومانی به‌ترتیب `< 50,000,000`, `50,000,000..200,000,000` و `> 200,000,000` اعمال می‌شوند. پاسخ API با eager loading روابط `user`, `bank`, `bankPlan`, `location.parent` و pagination سمت سرور برمی‌گردد.
